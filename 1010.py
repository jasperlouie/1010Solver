...
class Board:
    ...
    def place_piece(self, move, current_pieces):
        p = current_pieces[move.piece_index]
        loc = (move.x, move.y)
        if self.is_valid_move(p , loc):
            ...
            for y in full_rows:
                # print("clearing row {}!".format(y))
                for x in range(10):
                    self.matrix[x][y] = 0
            for x in full_cols:
                # print("clearing column {}!".format(x))
                for y in range(10):
                    self.matrix[x][y] = 0
            current_pieces.pop(move.piece_index)
...
class Move:
    def __init__(self, piece_index, x, y):
    ...
    def __init__(self, string):
        ...
        self.piece_index = int(move_list[0])-1
        self.x = int(move_list[1])
        self.y = int(move_list[2])
...
def play(get_move, verbose = True):
    ...
    while not game_over:
        ...
        while current_pieces != []:
            ...
            move = get_move(board, current_pieces)
            try:
                board.place_piece(move, current_pieces)
                print("Placed piece {} at {},{}".format(move.piece_index,move.x,move.y))
...
def human(board, current_pieces):
    move_input = input("Please input your move: ")
    # print(repr(move_input)
